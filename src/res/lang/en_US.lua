...
return {
    ...
    SEMANTIC_WORKFLOW_FRAMELIST_NUMERIC = 'Numeric',
...
This action cannot be undone.
    ...
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_TOOL_TIP = 'Duplicate sheet',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_MODE_BANNER = '-- DUPLICATE --',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_CANCEL = 'Cancel',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_CANCEL_TOOL_TIP = 'Cancel duplicate mode',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_INSERT_TOOL_TIP = 'Insert copy before this sheet',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_APPEND_TOOL_TIP = 'Append copy at end',
    SEMANTIC_WORKFLOW_PROJECT_RECENT = 'Recent:',
    SEMANTIC_WORKFLOW_PROJECT_NO_RECENT = 'No recent projects.',
    ...
    SEMANTIC_WORKFLOW_INPUTS_GOTO_PREVIEW = '→Preview',
    SEMANTIC_WORKFLOW_INPUTS_GOTO_PREVIEW_TOOL_TIP = 'Move active frame to the current preview frame position',
    SEMANTIC_WORKFLOW_INPUTS_UNDO = 'Undo',
    SEMANTIC_WORKFLOW_INPUTS_UNDO_TOOL_TIP = 'Undo last structural change (+Input / -Input / +Section / -Section)',
    SEMANTIC_WORKFLOW_INPUTS_REDO = 'Redo',
    SEMANTIC_WORKFLOW_INPUTS_REDO_TOOL_TIP = 'Redo undone structural change',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_LABEL = 'Label:',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_LABEL_TOOL_TIP = 'Optional label for this section, shown in the frame list',
    SEMANTIC_WORKFLOW_INPUTS_COLLAPSE_ALL = '-All',
    SEMANTIC_WORKFLOW_INPUTS_COLLAPSE_ALL_TOOL_TIP = 'Collapse all sections',
    SEMANTIC_WORKFLOW_INPUTS_EXPAND_ALL = '+All',
    SEMANTIC_WORKFLOW_INPUTS_EXPAND_ALL_TOOL_TIP = 'Expand all sections',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_UP = '^ Sec',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_UP_TOOL_TIP = 'Move this section up (swap with the one above)',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_DOWN = 'v Sec',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_DOWN_TOOL_TIP = 'Move this section down (swap with the one below)',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_COPY = 'Copy',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_COPY_TOOL_TIP = 'Copy this section to the clipboard',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_PASTE = 'Paste',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_PASTE_TOOL_TIP = 'Insert a copy of the clipboard section after this one',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_N = 'Repeat x:',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_INPUT = 'Insert N copies',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_INPUT_TOOL_TIP = 'Insert N copies of the selected frame after it',
    SEMANTIC_WORKFLOW_INPUTS_FIRST_SECTION = '|<',
    SEMANTIC_WORKFLOW_INPUTS_FIRST_SECTION_TOOL_TIP = 'Jump to the first section',
    SEMANTIC_WORKFLOW_INPUTS_PREV_SECTION = '<',
    SEMANTIC_WORKFLOW_INPUTS_PREV_SECTION_TOOL_TIP = 'Jump to the previous section',
    SEMANTIC_WORKFLOW_INPUTS_NEXT_SECTION = '>',
    SEMANTIC_WORKFLOW_INPUTS_NEXT_SECTION_TOOL_TIP = 'Jump to the next section',
    SEMANTIC_WORKFLOW_INPUTS_LAST_SECTION = '>|',
    SEMANTIC_WORKFLOW_INPUTS_LAST_SECTION_TOOL_TIP = 'Jump to the last section',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK = 'Insert N blank:',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK_BTN = 'Insert N blank',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK_TOOL_TIP = 'Insert N blank (neutral-input) frames after the selected one',
    ...
    SEMANTIC_WORKFLOW_INPUTS_SELECT_ALL_FRAMES = 'Sel All',
    SEMANTIC_WORKFLOW_INPUTS_SELECT_ALL_FRAMES_TOOL_TIP = 'Select all frames in this section for batch editing',
    SEMANTIC_WORKFLOW_INPUTS_DESELECT_ALL_FRAMES = 'Desel',
    SEMANTIC_WORKFLOW_INPUTS_DESELECT_ALL_FRAMES_TOOL_TIP = 'Deselect all frames',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FROM_GAME = 'From game',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FROM_GAME_TOOL_TIP = 'Copy the current in-game controller state to all selected frames',
    SEMANTIC_WORKFLOW_INPUTS_FIT_TIMEOUT = 'Fit',
    SEMANTIC_WORKFLOW_INPUTS_FIT_TIMEOUT_TOOL_TIP = 'Set timeout to the number of inputs in this section',
    SEMANTIC_WORKFLOW_INPUTS_TRIM_INPUTS = 'Trim',
    SEMANTIC_WORKFLOW_INPUTS_TRIM_INPUTS_TOOL_TIP = 'Remove inputs beyond the timeout limit',
    SEMANTIC_WORKFLOW_INPUTS_REVERSE_SECTION = 'Reverse',
    SEMANTIC_WORKFLOW_INPUTS_REVERSE_SECTION_TOOL_TIP = 'Reverse the order of all inputs in this section',
    SEMANTIC_WORKFLOW_INPUTS_SPLIT_SECTION = 'Split here',
    SEMANTIC_WORKFLOW_INPUTS_SPLIT_SECTION_TOOL_TIP = 'Split this section at the active frame into two sections',
    SEMANTIC_WORKFLOW_INPUTS_MERGE_SECTION = 'Merge next',
    SEMANTIC_WORKFLOW_INPUTS_MERGE_SECTION_TOOL_TIP = 'Merge this section with the next one',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FRAMES = 'Copy sel.',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FRAMES_TOOL_TIP = 'Copy selected frames to the frame clipboard',
    SEMANTIC_WORKFLOW_INPUTS_PASTE_FRAMES = 'Paste frm',
    ...
    SEMANTIC_WORKFLOW_INPUTS_INVERT_SEL = 'Invert sel.',
    ...
    SEMANTIC_WORKFLOW_CONTROL_SCALE_MAG_APPLY = 'Scale mag\xe2\x86\x92sel',
    SEMANTIC_WORKFLOW_CONTROL_SCALE_MAG_TOOL_TIP = 'Multiply goal_mag of all selected frames by this percentage (100 = no change, 50 = halve, 200 = double)',
    SEMANTIC_WORKFLOW_CONTROL_BATCH_BUTTON_TOOL_TIP = 'Toggle %s button on all selected frames at once',
    SEMANTIC_WORKFLOW_INPUTS_LOCK_AND_COLLAPSE = 'Lock\xe2\x86\x92next',
    SEMANTIC_WORKFLOW_INPUTS_LOCK_AND_COLLAPSE_TOOL_TIP = 'Lock this section, collapse it, and advance to the next section',
    SEMANTIC_WORKFLOW_CONTROL_SET_ANGLE_FROM_GAME = 'Yaw\xe2\x86\x92sel',
    SEMANTIC_WORKFLOW_CONTROL_SET_ANGLE_FROM_GAME_TOOL_TIP = 'Set goal_angle of all selected frames to Mario\'s current facing yaw and switch to match_angle mode',
    SEMANTIC_WORKFLOW_INPUTS_SAVE_TEMPLATE = 'Save tmpl',
    SEMANTIC_WORKFLOW_INPUTS_SAVE_TEMPLATE_TOOL_TIP = 'Save current section as a reusable template (uses section label as name)',
    SEMANTIC_WORKFLOW_INPUTS_LOAD_TEMPLATE_TOOL_TIP = 'Insert template "%s" as a new section after the current one',
    SEMANTIC_WORKFLOW_PREFERENCES_TEMPLATES = 'Templates',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_TEMPLATES = 'Clear all',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_TEMPLATES_TOOL_TIP = 'Delete all saved section templates',
    ...
    SEMANTIC_WORKFLOW_CONTROL_FLIP_Y_TOOL_TIP = 'Negate joystick Y for all selected frames',
    SEMANTIC_WORKFLOW_CONTROL_SET_MAG_ALL = 'Mag→all',
    SEMANTIC_WORKFLOW_CONTROL_SET_MAG_ALL_TOOL_TIP = 'Apply current magnitude to all selected frames',
    SEMANTIC_WORKFLOW_CONTROL_SWIM = 'Swim',
    SEMANTIC_WORKFLOW_CONTROL_FRAMEWALK = 'Frmwalk',
    SEMANTIC_WORKFLOW_CONTROL_FRAMEWALK_TOOL_TIP = 'Enable framewalk movement mode',
    SEMANTIC_WORKFLOW_PREFERENCES_FIT_ALL_TIMEOUTS = 'Fit all timeouts',
    SEMANTIC_WORKFLOW_PREFERENCES_FIT_ALL_TIMEOUTS_TOOL_TIP = 'Set timeout of every section to its input count',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_COLOR_TAGS = 'Clear color tags',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_COLOR_TAGS_TOOL_TIP = 'Remove all color tags from every section in this sheet',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_LABELS = 'Clear labels',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_LABELS_TOOL_TIP = 'Remove all labels from every section in this sheet',
    SEMANTIC_WORKFLOW_PREFERENCES_DESELECT_ALL = 'Deselect all frames',
    SEMANTIC_WORKFLOW_PREFERENCES_DESELECT_ALL_TOOL_TIP = 'Clear the editing selection on all frames in this sheet',
    SEMANTIC_WORKFLOW_PREFERENCES_UNLOCK_ALL = 'Unlock all',
    SEMANTIC_WORKFLOW_PREFERENCES_UNLOCK_ALL_TOOL_TIP = 'Remove the lock from all sections in this sheet',
    SEMANTIC_WORKFLOW_PREFERENCES_COLLAPSE_LOCKED = 'Collapse locked sections',
    SEMANTIC_WORKFLOW_PREFERENCES_COLLAPSE_LOCKED_TOOL_TIP = 'Collapse all locked sections to reduce visual clutter',
    SEMANTIC_WORKFLOW_PREFERENCES_EDIT_ENTIRE_STATE = 'Edit entire state',
    SEMANTIC_WORKFLOW_PREFERENCES_FAST_FORWARD = 'Fast Forward',
    SEMANTIC_WORKFLOW_PREFERENCES_AUTO_SAVE = 'Auto-save project',
    SEMANTIC_WORKFLOW_PREFERENCES_AUTO_SAVE_TOOL_TIP = 'Automatically save the project ~3 seconds after the last change',
