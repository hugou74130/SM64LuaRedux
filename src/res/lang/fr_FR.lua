...
return {
    ...
    SEMANTIC_WORKFLOW_FRAMELIST_NUMERIC = 'Numérique',
...
Cette action est irréversible.
    ...
    SEMANTIC_WORKFLOW_PROJECT_BASE_SHEET_TOOL_TIP = 'Cette feuille est bas\xc3\xa9e sur ',
    SEMANTIC_WORKFLOW_PROJECT_NO_BASE_SHEET_TOOL_TIP = 'Cette feuille n\'est pas bas\xc3\xa9e sur une autre feuille.',
    SEMANTIC_WORKFLOW_PROJECT_SET_BASE_SHEET_TOOL_TIP = 'D\xc3\xa9finir comme feuille de base pour ',
    ...
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_TOOL_TIP = 'Dupliquer la feuille',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_MODE_BANNER = '-- DUPLICATION --',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_CANCEL = 'Annuler',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_CANCEL_TOOL_TIP = 'Annuler le mode duplication',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_INSERT_TOOL_TIP = 'Insérer la copie avant cette feuille',
    SEMANTIC_WORKFLOW_PROJECT_DUPLICATE_APPEND_TOOL_TIP = 'Ajouter la copie à la fin',
    SEMANTIC_WORKFLOW_PROJECT_RECENT = 'Récents :',
    SEMANTIC_WORKFLOW_PROJECT_NO_RECENT = 'Aucun projet récent.',
    ...
    SEMANTIC_WORKFLOW_INPUTS_GOTO_PREVIEW = '→Aperçu',
    SEMANTIC_WORKFLOW_INPUTS_GOTO_PREVIEW_TOOL_TIP = 'Déplacer la frame active à la position de la frame d\'aperçu',
    SEMANTIC_WORKFLOW_INPUTS_UNDO = 'Annuler',
    SEMANTIC_WORKFLOW_INPUTS_UNDO_TOOL_TIP = 'Annuler le dernier changement structurel (+Input / -Input / +Section / -Section)',
    SEMANTIC_WORKFLOW_INPUTS_REDO = 'Rétablir',
    SEMANTIC_WORKFLOW_INPUTS_REDO_TOOL_TIP = 'Rétablir le changement annulé',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_LABEL = 'Label :',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_LABEL_TOOL_TIP = 'Label optionnel pour cette section, affiché dans la liste',
    SEMANTIC_WORKFLOW_INPUTS_COLLAPSE_ALL = '-Tout',
    SEMANTIC_WORKFLOW_INPUTS_COLLAPSE_ALL_TOOL_TIP = 'Réduire toutes les sections',
    SEMANTIC_WORKFLOW_INPUTS_EXPAND_ALL = '+Tout',
    SEMANTIC_WORKFLOW_INPUTS_EXPAND_ALL_TOOL_TIP = 'Développer toutes les sections',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_UP = '^ Sec',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_UP_TOOL_TIP = 'Remonter cette section (échanger avec la précédente)',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_DOWN = 'v Sec',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_MOVE_DOWN_TOOL_TIP = 'Descendre cette section (échanger avec la suivante)',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_COPY = 'Copier',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_COPY_TOOL_TIP = 'Copier cette section dans le presse-papier',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_PASTE = 'Coller',
    SEMANTIC_WORKFLOW_INPUTS_SECTION_PASTE_TOOL_TIP = 'Insérer une copie de la section du presse-papier après celle-ci',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_N = 'Répéter x :',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_INPUT = 'Insérer N copies',
    SEMANTIC_WORKFLOW_INPUTS_REPEAT_INPUT_TOOL_TIP = 'Insérer N copies de la frame sélectionnée après elle',
    SEMANTIC_WORKFLOW_INPUTS_FIRST_SECTION = '|<',
    SEMANTIC_WORKFLOW_INPUTS_FIRST_SECTION_TOOL_TIP = 'Aller à la première section',
    SEMANTIC_WORKFLOW_INPUTS_PREV_SECTION = '<',
    SEMANTIC_WORKFLOW_INPUTS_PREV_SECTION_TOOL_TIP = 'Aller à la section précédente',
    SEMANTIC_WORKFLOW_INPUTS_NEXT_SECTION = '>',
    SEMANTIC_WORKFLOW_INPUTS_NEXT_SECTION_TOOL_TIP = 'Aller à la section suivante',
    SEMANTIC_WORKFLOW_INPUTS_LAST_SECTION = '>|',
    SEMANTIC_WORKFLOW_INPUTS_LAST_SECTION_TOOL_TIP = 'Aller à la dernière section',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK = 'Insérer N vides :',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK_BTN = 'Insérer N vides',
    SEMANTIC_WORKFLOW_INPUTS_INSERT_N_BLANK_TOOL_TIP = 'Insérer N frames vides (sans input) après la frame sélectionnée',
    ...
    SEMANTIC_WORKFLOW_INPUTS_SELECT_ALL_FRAMES = 'Sél. Tout',
    SEMANTIC_WORKFLOW_INPUTS_SELECT_ALL_FRAMES_TOOL_TIP = 'Sélectionner toutes les frames de cette section pour édition en masse',
    SEMANTIC_WORKFLOW_INPUTS_DESELECT_ALL_FRAMES = 'Désél.',
    SEMANTIC_WORKFLOW_INPUTS_DESELECT_ALL_FRAMES_TOOL_TIP = 'Désélectionner toutes les frames',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FROM_GAME = 'Du jeu',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FROM_GAME_TOOL_TIP = 'Copier l\'état actuel du contrôleur dans toutes les frames sélectionnées',
    SEMANTIC_WORKFLOW_INPUTS_FIT_TIMEOUT = 'Ajuster',
    SEMANTIC_WORKFLOW_INPUTS_FIT_TIMEOUT_TOOL_TIP = 'Définir le délai égal au nombre d\'inputs de cette section',
    SEMANTIC_WORKFLOW_INPUTS_TRIM_INPUTS = 'Élaguer',
    SEMANTIC_WORKFLOW_INPUTS_TRIM_INPUTS_TOOL_TIP = 'Supprimer les inputs au-delà du délai limite',
    SEMANTIC_WORKFLOW_INPUTS_REVERSE_SECTION = 'Inverser',
    SEMANTIC_WORKFLOW_INPUTS_REVERSE_SECTION_TOOL_TIP = 'Inverser l\'ordre de tous les inputs de cette section',
    SEMANTIC_WORKFLOW_INPUTS_SPLIT_SECTION = 'Diviser ici',
    SEMANTIC_WORKFLOW_INPUTS_SPLIT_SECTION_TOOL_TIP = 'Diviser cette section à la frame active en deux sections',
    SEMANTIC_WORKFLOW_INPUTS_MERGE_SECTION = 'Fusionner +',
    SEMANTIC_WORKFLOW_INPUTS_MERGE_SECTION_TOOL_TIP = 'Fusionner cette section avec la suivante',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FRAMES = 'Copier sél.',
    SEMANTIC_WORKFLOW_INPUTS_COPY_FRAMES_TOOL_TIP = 'Copier les frames sélectionnées dans le presse-papiers de frames',
    SEMANTIC_WORKFLOW_INPUTS_PASTE_FRAMES = 'Coller frm',
    ...
    SEMANTIC_WORKFLOW_INPUTS_INVERT_SEL = 'Inv. sél.',
    SEMANTIC_WORKFLOW_INPUTS_INVERT_SEL_TOOL_TIP = 'Inverser la sélection d\'édition de chaque frame dans cette section',
    SEMANTIC_WORKFLOW_INPUTS_STATS_NO_SEL = 'aucune sél.',
    SEMANTIC_WORKFLOW_CONTROL_ANGLE_OFFSET = '\xce\x94angle',
    SEMANTIC_WORKFLOW_CONTROL_ANGLE_OFFSET_APPLY = 'App. \xce\x94\xe2\x86\x92sél',
    SEMANTIC_WORKFLOW_CONTROL_ANGLE_OFFSET_APPLY_TOOL_TIP = 'Ajouter le décalage d\'angle au goal_angle de toutes les frames sélectionnées dans les sections non verrouillées',
    SEMANTIC_WORKFLOW_CONTROL_ANGLE_SNAP = 'Arron.\xe2\x86\x92sél',
    SEMANTIC_WORKFLOW_CONTROL_ANGLE_SNAP_TOOL_TIP = 'Arrondir le goal_angle des frames sélectionnées au multiple le plus proche de \xce\x94angle',
    SEMANTIC_WORKFLOW_CONTROL_SCALE_MAG = 'Mag %',
    SEMANTIC_WORKFLOW_CONTROL_SCALE_MAG_APPLY = '\xc3\x89ch.\xe2\x86\x92s\xc3\xa9l',
    SEMANTIC_WORKFLOW_CONTROL_SCALE_MAG_TOOL_TIP = 'Multiplier le goal_mag de toutes les frames sélectionnées par ce pourcentage (100 = inchangé, 50 = moitié, 200 = double)',
    SEMANTIC_WORKFLOW_CONTROL_BATCH_BUTTON_TOOL_TIP = 'Basculer le bouton %s sur toutes les frames sélectionnées à la fois',
    SEMANTIC_WORKFLOW_INPUTS_LOCK_AND_COLLAPSE = 'Verr.\xe2\x86\x92suiv',
    SEMANTIC_WORKFLOW_INPUTS_LOCK_AND_COLLAPSE_TOOL_TIP = 'Verrouiller cette section, la réduire et passer à la suivante',
    SEMANTIC_WORKFLOW_CONTROL_SET_ANGLE_FROM_GAME = 'Yaw\xe2\x86\x92sél',
    SEMANTIC_WORKFLOW_CONTROL_SET_ANGLE_FROM_GAME_TOOL_TIP = 'Définir le goal_angle de toutes les frames sélectionnées sur le yaw frontal actuel de Mario et passer en mode match_angle',
    SEMANTIC_WORKFLOW_INPUTS_SAVE_TEMPLATE = 'Sauv. modèle',
    SEMANTIC_WORKFLOW_INPUTS_SAVE_TEMPLATE_TOOL_TIP = 'Sauvegarder la section actuelle comme modèle réutilisable (utilise le label comme nom)',
    SEMANTIC_WORKFLOW_INPUTS_LOAD_TEMPLATE_TOOL_TIP = 'Insérer le modèle "%s" comme nouvelle section après la section actuelle',
    SEMANTIC_WORKFLOW_PREFERENCES_TEMPLATES = 'Modèles',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_TEMPLATES = 'Tout effacer',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_TEMPLATES_TOOL_TIP = 'Supprimer tous les modèles de sections sauvegardés',
    ...
    SEMANTIC_WORKFLOW_CONTROL_FLIP_Y_TOOL_TIP = 'Inverser le joystick Y pour toutes les frames sélectionnées',
    SEMANTIC_WORKFLOW_CONTROL_SET_MAG_ALL = 'Mag→tous',
    SEMANTIC_WORKFLOW_CONTROL_SET_MAG_ALL_TOOL_TIP = 'Appliquer la magnitude actuelle à toutes les frames sélectionnées',
    SEMANTIC_WORKFLOW_CONTROL_SWIM = 'Nage',
    SEMANTIC_WORKFLOW_CONTROL_FRAMEWALK = 'Pasframe',
    SEMANTIC_WORKFLOW_CONTROL_FRAMEWALK_TOOL_TIP = 'Activer le mode de déplacement framewalk',
    SEMANTIC_WORKFLOW_PREFERENCES_FIT_ALL_TIMEOUTS = 'Ajuster tous les délais',
    SEMANTIC_WORKFLOW_PREFERENCES_FIT_ALL_TIMEOUTS_TOOL_TIP = 'Définir le délai de chaque section égal à son nombre d\'inputs',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_COLOR_TAGS = 'Effacer les couleurs',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_COLOR_TAGS_TOOL_TIP = 'Supprimer toutes les couleurs de toutes les sections de cette feuille',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_LABELS = 'Effacer les étiquettes',
    SEMANTIC_WORKFLOW_PREFERENCES_CLEAR_LABELS_TOOL_TIP = 'Supprimer toutes les étiquettes de toutes les sections de cette feuille',
    SEMANTIC_WORKFLOW_PREFERENCES_DESELECT_ALL = 'D\xc3\xa9s\xc3\xa9l. tout',
    SEMANTIC_WORKFLOW_PREFERENCES_DESELECT_ALL_TOOL_TIP = 'Effacer la sélection d\'édition sur toutes les frames de cette feuille',
    SEMANTIC_WORKFLOW_PREFERENCES_UNLOCK_ALL = 'Tout déverr.',
    SEMANTIC_WORKFLOW_PREFERENCES_UNLOCK_ALL_TOOL_TIP = 'Supprimer le verrou de toutes les sections de cette feuille',
    SEMANTIC_WORKFLOW_PREFERENCES_COLLAPSE_LOCKED = 'Réduire les sections verrouillées',
    SEMANTIC_WORKFLOW_PREFERENCES_COLLAPSE_LOCKED_TOOL_TIP = 'Réduire toutes les sections verrouillées pour réduire l\'encombrement visuel',
    ...
    SEMANTIC_WORKFLOW_PREFERENCES_AUTO_SAVE = 'Sauvegarde automatique',
    SEMANTIC_WORKFLOW_PREFERENCES_AUTO_SAVE_TOOL_TIP = 'Sauvegarder automatiquement le projet ~3 secondes après le dernier changement',
